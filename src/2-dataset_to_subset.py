...
def run():
    ...
    with h5py.File(dataset_filepath, 'r') as f1, h5py.File('HWDB1.1subset.hdf5', 'w') as f2:
        ...
        trn_x = trn_grp.create_dataset('x', (trn_size, 64, 64, 1), dtype='uint8')
        trn_y = trn_grp.create_dataset('y', (trn_size, 200),       dtype='uint16')

        vld_size = len(indexes)-trn_size # 7783
        vld_grp = f2.create_group('vld')
        vld_x = vld_grp.create_dataset('x', (vld_size, 64, 64, 1), dtype='uint8')
        ...
        # find allowed characters in the data set
        indexes = [i for i in range(TST_SIZE) if f1['tst/tagcode'][i][0] in allowed]
        new_size = len(indexes) # 11947

        # create the subset
        tst_grp = f2.create_group('tst')
        tst_x = tst_grp.create_dataset('x', (new_size, 64, 64, 1), dtype='uint8')
